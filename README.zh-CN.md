...
目前计划将 **RenderGraph（渲染图）** 优化为真正的 **并行化架构**，  
并扩展 **NodeGraph（节点图）** 可视化系统，使其支持更加多样化与动态的节点结构。

在此阶段，其他计划中的特性将会暂时 **暂停开发**，

**目标计划包括：**
- 支持在 NodeGraph 中 **动态创建与删除 RenderTarget (RT)**。
- 允许通过交互式拖拽实现 **任意 RT 节点之间的动态重连**。
- 将渲染图数据抽取为 **自定义脚本层**，可自动生成 GLSL 代码与 Descriptor Set 绑定关系。
- 实现 **运行时修改整个渲染管线** 的能力，通过该脚本系统进行控制。
- 探索 **基于 AI 的渲染优化**，由 AI 模块自动分析每个 RT Pass 的执行时间，并自动调整渲染顺序以获得最优性能。
