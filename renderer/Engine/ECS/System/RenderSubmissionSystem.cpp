...
namespace sc {
    ...
    auto RenderSubmissionSystem::flow(vot::CommandBuffer &cmd) -> void {
        ...
        uRenderGraph->addPass({
            ...
            .execute = [&]{
                cmd.setRenderArea_(vot::Resolutions::eQHDExtent);
                draw_meshes(yic::renderLibrary->GP_Basic, ecs.view<const vot::RenderVisibleTag, const vot::RenderComponent>(entt::exclude<vot::MMDTag>));
        ...
        uRenderGraph->addPass({
            ...
            .execute = [&]{
                cmd.setRenderArea_(vot::Resolutions::eQHDExtent);
                draw_meshes_shadowMap(yic::renderLibrary->GP_ShadowMap_Basic, ecs.view<const vot::RenderVisibleTag, const vot::RenderComponent>(entt::exclude<vot::MMDTag>));
        ...
        uRenderGraph->addPass({
            ...
            .inputs = { yic::renderLibrary->RT_ShadowMap },
            .execute = draw_volumetric_clouds
        });
        uRenderGraph->addPass({
            .target = yic::renderLibrary->RT_Volumetric_Fog,
            .inputs = { yic::renderLibrary->RT_Volumetric_Clouds },
            .execute = draw_volumetric_fog
        });
        uRenderGraph->addPass({
            .target = yic::renderLibrary->RT_RayTracing,
            .inputs = { yic::renderLibrary->RT_Volumetric_Fog },
            .drawci = drawci_RTShadow,
            .execute = draw_RTShadow,
        });
        uRenderGraph->addPass({
            .target = yic::renderLibrary->RT_Post,
            .inputs = { yic::renderLibrary->RT_RayTracing },
            .execute = draw_post,
        });

        uRenderGraph->exe(cmd);
    }
