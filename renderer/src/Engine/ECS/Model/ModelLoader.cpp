...
namespace sc {
    ...
    auto ModelLoader::processMesh(const aiMesh *aiMesh, Mesh& mesh, const aiVector3D& center) -> void {
        ...
        mesh.vertBuf = yic::Allocator::allocBufStaging(sizeof(Vertex) * mesh.vertices.size(), mesh.vertices.data(), vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress | vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR);
        mesh.indexBuf = yic::Allocator::allocBufStaging(sizeof(uint32_t) * mesh.indices.size(), mesh.indices.data(), vk::BufferUsageFlagBits::eIndexBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress | vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR);

        mesh.bufAddr.vertAddr = yic::Allocator::getBufAddr(mesh.vertBuf);
        mesh.bufAddr.indexAddr = yic::Allocator::getBufAddr(mesh.indexBuf);

        mesh.addrBuf = yic::Allocator::allocBufStaging(sizeof(MeshBufAddress), &mesh.bufAddr, vk::BufferUsageFlagBits::eStorageBuffer);
    }
